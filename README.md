...
**诗成写作 (ShiCheng Writer)** 是一款坚持“本地优先”的桌面应用程序，我们高度尊重并保护您的个人隐私及创作数据。

1.  **数据完全本地化**:
    * 您所有的书籍内容、大纲、素材设定及灵感记录均存储在您本地计算机的 SQLite 数据库文件 (`ShiCheng_Writer.db`) 中。
    * 软件**不会**将您的创作内容上传至任何云端服务器、第三方平台或开发者后台。

2.  **无网络依赖与追踪**:
    * 本软件的核心功能完全**离线运行**，不需要互联网连接。
    * 我们**不收集**任何用户行为数据、设备指纹或使用统计（Telemetry）。

3.  **备份文件的归属**:
    * 系统自动生成的快照与归档文件均存储在您项目目录下的 `backups/` 文件夹中。这些文件仅存在于您的设备上，完全由您掌控。

4.  **数据安全责任**:
    * 由于数据不经过云端，请您自行负责本地设备的安全防护。
    * 建议定期将 `ShiCheng_Writer.db` 文件或 `backups` 文件夹复制到外部存储设备或您的私有云盘中，以防范硬盘物理损坏带来的风险。
